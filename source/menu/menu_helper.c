...
void Menu_DrawStringCentered (int x, int y, char* text, int r, int g, int b, int a)
{
	...
	x = x - (getTextWidth(text, vid.scale)/2);

	Draw_ColoredString (x, y, text, r, g, b, a, vid.scale);
}
...
void Menu_DrawDivider (float order)
{
	...
	if (order < 0) {
		...
		y_pos = 30 + (order*y_factor) - (_CHAR_HEIGHT/2);
...
void Menu_DrawTitle (char *title_name, int color)
{
	...
	switch (color) {
		case MENU_COLOR_WHITE:
			Draw_ColoredString (x_pos, y_pos, title_name, 255, 255, 255, 255, vid.scale*2);
			break;
		case MENU_COLOR_YELLOW:
			Draw_ColoredString (x_pos, y_pos, title_name, 255, 255, 0, 255, vid.scale*2);
			break;
	}
}
...
void Menu_DrawSelectionBox (int x_pos, int y_pos)
{
	...
	int border_height = (horizontal_border_height*2) + (_CHAR_HEIGHT);
...
void Menu_DrawMapBorder (int x_pos, int y_pos, int image_width, int image_height)
{
	...
	// Top
	Menu_DrawFill (x_pos, y_pos, (image_width/vid.scale)+border_side_width, small_bar_height, 130, 130, 130, 255);
	// Left Side
	Menu_DrawFill (x_pos, y_pos, border_side_width, (image_height/vid.scale), 130, 130, 130, 255);
	// Right Side
	Menu_DrawFill (x_pos+(image_width/vid.scale), y_pos, border_side_width, (image_height/vid.scale), 130, 130, 130, 255);
	// Bottom
	if (current_frame.point_y == UI_ANCHOR_TOP) {
		Menu_DrawFill (x_pos, y_pos+(image_height/vid.scale), (image_width/vid.scale)+border_side_width, small_bar_height, 130, 130, 130, 255);
	} else if (current_frame.point_y == UI_ANCHOR_BOTTOM) {
		Menu_DrawFill (x_pos, y_pos-(image_height/vid.scale), (image_width/vid.scale)+border_side_width, small_bar_height, 130, 130, 130, 255);
	}
...
void Menu_DrawMapPanel (void)
{
	...
	UI_SetAlignment (UI_ANCHOR_LEFT, UI_ANCHOR_TOP);
	// Big box
	Menu_DrawFill(x_pos, y_pos, (vid.width - (x_pos*vid.scale))/vid.scale, (vid.height - (y_pos*(vid.scale*2)))/vid.scale, 0, 0, 0, 118);
	// Yellow bar
	Menu_DrawFill(x_pos, y_pos, 2, (vid.height - (y_pos*(vid.scale*2)))/vid.scale, 255, 255, 0, 200);
}

void Menu_DrawSubMenu (char *line_one, char *line_two)
{
	int y_pos = (vid.height/3)/vid.scale;
	...
	// Quit/Restart Text
    Menu_DrawStringCentered (0, y_pos + (_CHAR_HEIGHT*vid.scale), line_one, 255, 255, 255, 255);
    // Lose progress text
...
void Menu_DrawButton (int order, int button_index, char* button_name, char* button_summary, void *on_activate)
{
	...
	if (Menu_IsButtonHovered(button_index)) {
		...
		// Draw button string
		Menu_DrawString (x_pos, y_pos, button_name, 255, 0, 0, 255, vid.scale, UI_FLIPTEXTPOS);
		...
		Menu_DrawString (x_pos, y_pos, button_name, 255, 255, 255, 255, vid.scale, UI_FLIPTEXTPOS);
	}
}
...
void Menu_DrawMapButton (int order, int button_index, int usermap_index, int map_category, char* bsp_name, void *on_activate)
{
	...
	char *button_name;
	char *final_name;
	int desc_y_factor = 10*vid.scale;
	...
	int x_pos = map_region_center - ((image_width/vid.scale)/2);
	...
	if (Menu_IsButtonHovered(button_index)) {
		...
		Menu_DrawString((x_pos + 4 + menu_badges_width)/vid.scale, (y_pos + image_height - (menu_badges_height/2) - (_CHAR_HEIGHT/2))/vid.scale, badge_name, 255, 255, 0, 255, vid.scale, 0);
		...
		for (int j = 0; j < 8; j++) {
			Menu_DrawStringCentered ((x_pos + (image_width/2)), (y_pos + image_height + small_bar_height + ((_CHAR_HEIGHT/2)*vid.scale) + j*desc_y_factor), custom_maps[index].map_desc[j], 255, 255, 255, 255);
		}
#endif
		// Draw map author
		UI_SetAlignment (UI_ANCHOR_LEFT, UI_ANCHOR_BOTTOM);
...
void Menu_DrawOptionSlider(int order, int button_index, int min_option_value, int max_option_value, cvar_t option, char* option_string, qboolean zero_to_one, qboolean draw_option_string, float increment_amount)
{
	...
	if (draw_option_string) {
		Menu_DrawString(x_pos + slider_box_width + _CHAR_WIDTH, y_pos - 2, _option_string, 255, 255, 255, 255, vid.scale, 0);
	}
}
...
void Menu_DrawLobbyInfo (char* bsp_name, char* info_gamemode, char* info_difficulty, char* info_startround, char* info_magic, char* info_headshotonly, char* info_fastrounds, char* info_hordesize)
{
	...
	int map_region_center = (150 + (((vid.width-150)/2)+2));
	int image_x_pos = map_region_center - ((image_width/vid.scale)/2);
	int image_y_pos = ((big_bar_height+small_bar_height)+(_CHAR_HEIGHT)/2) + (image_height/vid.scale);

	int left_column_x = map_region_center - ((image_width/vid.scale)/2) + (20*vid.scale);
	int left_column_y = 30*vid.scale;

	int y_offset = 10*vid.scale;
	int y_newline = 25*vid.scale;
	int x_newline = image_width/1.33;

	int line_x = map_region_center - ((image_width/vid.scale)/2) - 8;
	...
	Menu_DrawFill (line_x/vid.scale, (left_column_y + (y_offset*2))/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	Menu_DrawFill ((line_x + x_newline)/vid.scale, (left_column_y + (y_offset*2))/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	Menu_DrawFill (line_x/vid.scale, (left_column_y + (y_offset*2) + y_newline)/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	Menu_DrawFill ((line_x + x_newline)/vid.scale, (left_column_y + (y_offset*2) + y_newline)/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	Menu_DrawFill (line_x/vid.scale, (left_column_y + (y_offset*2) + (y_newline*2))/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	Menu_DrawFill ((line_x + x_newline)/vid.scale, (left_column_y + (y_offset*2) + (y_newline*2))/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	Menu_DrawFill (line_x/vid.scale, (left_column_y + (y_offset*2) + (y_newline*3))/vid.scale, 55, (small_bar_height/2), 130, 130, 130, 255);
	...
	// Draw border around map image
	Menu_DrawMapBorder (image_x_pos/vid.scale, image_y_pos, image_width, image_height);
	// Map Name (Pretty)
	Menu_DrawStringCentered (image_x_pos + (image_width/2), image_y_pos - image_height + _CHAR_HEIGHT, custom_maps[i].map_name_pretty, 255, 255, 0, 255);
}
...
void Menu_DrawBuildDate (void)
{
	...
	Menu_DrawString(0, y_pos, game_build_date, 255, 255, 255, 255, vid.scale, UI_FLIPTEXTPOS);
	// Welcome text
	Menu_DrawString(0, y_pos + y_offset, welcome_text, 255, 255, 0, 255, vid.scale, UI_FLIPTEXTPOS);
}
...
void Menu_DrawCreditHeader (int order, char *header)
{
	...
	Menu_DrawString(x_pos, y_pos, header, 255, 255, 0, 255, vid.scale, UI_FLIPTEXTPOS);
}
...
void Menu_OSK_Draw (void)
{
	...
	for(i=0;i<=MAX_Y;i++) {
		Menu_DrawString (x_pos, y_pos+(_CHAR_HEIGHT*i), osk_text[i], 255, 255, 255, 255, vid.scale, 0);
		
		if (i == 0) {
			Menu_DrawString (x_pos+(x_value)-88, y_pos+(_CHAR_HEIGHT*i), "CONFIRM", 255, 255, 0, 255, vid.scale, 0);
			Menu_DrawPicStretch (x_pos+(x_value)-20, y_pos+(_CHAR_HEIGHT*i), osk_button[0], 8, 8);
		} else if (i == 2) {
			Menu_DrawString (x_pos+(x_value)-88, y_pos+(_CHAR_HEIGHT*i), "CANCEL", 255, 255, 0, 255, vid.scale, 0);
			Menu_DrawPicStretch (x_pos+(x_value)-20, y_pos+(_CHAR_HEIGHT*i), osk_button[1], 8, 8);
		} else if (i == 4) {
			Menu_DrawString (x_pos+(x_value)-88, y_pos+(_CHAR_HEIGHT*i), "DELETE", 255, 255, 0, 255, vid.scale, 0);
			Menu_DrawPicStretch (x_pos+(x_value)-20, y_pos+(_CHAR_HEIGHT*i), osk_button[2], 8, 8);
		} else if (i == 6) {
			Menu_DrawString (x_pos+(x_value)-88, y_pos+(_CHAR_HEIGHT*i), "ADD CHAR", 255, 255, 0, 255, vid.scale, 0);
			Menu_DrawPicStretch (x_pos+(x_value)-20, y_pos+(_CHAR_HEIGHT*i), osk_button[3], 8, 8);
		}
	}
	// Side bar
	...
	if (text_len > MAX_CHAR_LINE) {
		...
		Menu_DrawString (x_pos, y_pos+4+(_CHAR_HEIGHT*(MAX_Y+2)), oneline, 255, 0, 0, 255, vid.scale, 0);

		strncpy(oneline,osk_buffer+MAX_CHAR_LINE, text_len - MAX_CHAR_LINE);
		oneline[text_len - MAX_CHAR_LINE] = '\0';

		// Current input char
		Menu_DrawString (x_pos+(_CHAR_WIDTH/2), y_pos+4+(_CHAR_HEIGHT*(MAX_Y+3)), oneline, 255, 255, 255, 255, vid.scale, 0);
		// Current cursor
		Menu_DrawFill (x_pos+((_CHAR_WIDTH)*(text_len)), y_pos+4+(_CHAR_HEIGHT*(MAX_Y+2)), 1, _CHAR_HEIGHT, 255, 0, 0, 255);
	} else {
		// Current input char
		Menu_DrawString (x_pos+(_CHAR_WIDTH/2), y_pos+4+(_CHAR_HEIGHT*(MAX_Y+2)), osk_buffer, 255, 255, 255, 255, vid.scale, 0);
		// Current cursor
		Menu_DrawFill (x_pos+((_CHAR_WIDTH)*(text_len)), y_pos+4+(_CHAR_HEIGHT*(MAX_Y+2)), 1, _CHAR_HEIGHT, 255, 0, 0, 255);
	}
	...
	Draw_ColoredString(hover_x, y_pos + (osk_pos_y * _CHAR_HEIGHT), selected_char, 255, 0, 0, 255, vid.scale);
}
#endif