...
//     uint32_t *anim_model_framegroup_start_frame = get_unpacked_member(anim_model->framegroup_start_frame, anim_model);
//     uint32_t *anim_model_framegroup_n_frames = get_unpacked_member(anim_model->framegroup_n_frames, anim_model);
//     float *anim_model_framegroup_fps = get_unpacked_member(anim_model->framegroup_fps, anim_model);
//     bool *anim_model_framegroup_loop = get_unpacked_member(anim_model->framegroup_loop, anim_model);
...
//     int16_t *bone_parent_idx = get_unpacked_member(skeleton->model->bone_parent_idx, skeleton->model);
//     mat3x4_t *bone_rest_transforms = get_unpacked_member(skeleton->model->bone_rest_transforms, skeleton->model);
//     mat3x4_t *inv_bone_rest_transforms = get_unpacked_member(skeleton->model->inv_bone_rest_transforms, skeleton->model);
...
void set_skeleton_bone_states_from_model(skeletal_skeleton_t *skeleton, skeletal_model_t *skel_model) {
    ...
    // bool *model_bone_hitbox_enabled = get_unpacked_member(skel_model->bone_hitbox_enabled, skel_model);
    // vec3_t *model_bone_hitbox_ofs = get_unpacked_member(skel_model->bone_hitbox_ofs, skel_model);
    // vec3_t *model_bone_hitbox_scale = get_unpacked_member(skel_model->bone_hitbox_scale, skel_model);
    // int *model_bone_hitbox_tag = get_unpacked_member(skel_model->bone_hitbox_tag, skel_model);
...
//     // --------------------------------------------------------------------
//     // json json_data = json::parse(iqm_info_json_str, NULL, false, true); // excpetions=false, ignore_comments=true
...
//     json json_data;
//     try {
//         json_data = json::parse(iqm_info_json_str, NULL, true, true); // excpetions=true, ignore_comments=true
...
//                 skel_model->materials[material_idx] = (skeletal_material_t*) malloc(sizeof(skeletal_material_t) * n_skins);
//                 int skin_idx = 0;
//                 load_material_json_info(&(skel_model->materials[material_idx][skin_idx]), NULL, iqm_info_json_file, material_name);
//             }
//         }
...
int cl_get_camera_bone_view_transform(entity_t *view_ent, mat3x4_t camera_anim_view_matrix_out) {
    ...
    // // mat3x4_t *inv_bone_rest_transforms = get_unpacked_member(skel_model->inv_bone_rest_transforms, skel_model);
...
const iqm_model_vertex_arrays_t load_iqm_model_vertex_arrays(const iqm_header_t *iqm_header) {
    ...
    iqm_vertex_arrays.verts_bone_weights    = (float*)    malloc(sizeof(float) * 4 * iqm_header->n_verts);
    iqm_vertex_arrays.verts_bone_idxs       = (uint8_t*)  malloc(sizeof(uint8_t) * 4 * iqm_header->n_verts);
#ifdef IQM_LOAD_NORMALS
    ...
    iqm_parse_float_array(iqm_header, raw_verts_bone_weights, iqm_vertex_arrays.verts_bone_weights,  iqm_header->n_verts, 4, (float*) &default_bone_weights);
    iqm_parse_uint8_array(iqm_header, raw_verts_bone_idxs,    iqm_vertex_arrays.verts_bone_idxs,     iqm_header->n_verts, 4, max_bone_idx);
#ifdef IQM_LOAD_NORMALS
...
// 
// Sums up the data pointer to by all points and returns the total total number 
// of bytes required to store an unpacked skeletal_model_t as a packed contigous
// block of memory
// 
uint32_t count_unpacked_skel_model_n_bytes(skeletal_model_t *skel_model) {
    uint32_t skel_model_n_bytes = 0;
    skel_model_n_bytes += sizeof(skeletal_model_t);

    skel_model_n_bytes += count_unpacked_skel_model_meshes_n_bytes(skel_model);
...
//
// Given an unpacked `skel_model`, debug prints all of its data
//
void debug_print_unpacked_skel_model(skeletal_model_t *skel_model) {
    // ------------------------------------------------------------------------
    ...
    // TODO - Call debug function that prints detailed mesh data
...
void Mod_LoadIQMModel (model_t *model, void *buffer) {
    ...
    // debug_print_unpacked_skel_model(unpacked_skel_model);


#ifdef IQMDEBUG_LOADIQM_RELOCATE
    Con_Printf("About to calculate skeletal model size...\n");
#endif // IQMDEBUG_LOADIQM_RELOCATE
    uint32_t skel_model_n_bytes = count_unpacked_skel_model_n_bytes(unpacked_skel_model);
#ifdef IQMDEBUG_LOADIQM_RELOCATE
...
char *sv_skel_get_bonename(int skel_idx, int bone_idx) {
    ...
    // char **bone_names = get_unpacked_member(sv_skeleton->model->bone_name, sv_skeleton->model);
    // char *bone_name = get_unpacked_member(bone_names[bone_idx], sv_skeleton->model);
    // return bone_name;
}
...
int16_t sv_skel_get_boneparent(int skel_idx, int bone_idx) {
    ...
    // skeletal_skeleton_t *sv_skeleton = &(sv_skeletons[skel_idx]);
    // int16_t *bone_parent_idxs = get_unpacked_member(sv_skeleton->model->bone_parent_idx, sv_skeleton->model);
...
int16_t sv_skel_find_bone(int skel_idx, char *bone_name) {
    ...
    //     char *cur_bone_name = get_unpacked_member(bone_names[i], sv_skeleton->model);
...
void PF_getmovedistance(void) {
    ...
    // uint32_t *framegroup_start_frame = get_unpacked_member(anim_skel_model->framegroup_start_frame, anim_skel_model);
    // uint32_t *framegroup_n_frames = get_unpacked_member(anim_skel_model->framegroup_n_frames, anim_skel_model);
    // float *framegroup_fps = get_unpacked_member(anim_skel_model->framegroup_fps, anim_skel_model);
    // bool *framegroup_loop = get_unpacked_member(anim_skel_model->framegroup_loop, anim_skel_model);

    // // Movement distance for all frames (cumsum-ed within each framegroup)
    // float *frames_move_dist = get_unpacked_member(anim_skel_model->frames_move_dist, anim_skel_model);
...
qboolean sv_intersect_skeletal_model_ent(edict_t *ent, vec3_t ray_start, vec3_t ray_end, float *trace_fraction, int *trace_bone_idx, int *trace_bone_hitbox_tag) {
    ...
    // // char **bone_names = get_unpacked_member(sv_skeleton->model->bone_name, sv_skeleton->model);
    // // bool *bone_hitbox_enabled = get_unpacked_member(sv_skeleton->model->bone_hitbox_enabled, sv_skeleton->model);
    // // vec3_t *bone_hitbox_ofs = get_unpacked_member(sv_skeleton->model->bone_hitbox_ofs, sv_skeleton->model);
    // // vec3_t *bone_hitbox_scale = get_unpacked_member(sv_skeleton->model->bone_hitbox_scale, sv_skeleton->model);
    // // int *bone_hitbox_tag = get_unpacked_member(sv_skeleton->model->bone_hitbox_tag, sv_skeleton->model);
    ...
    //     // char *bone_name = get_unpacked_member(bone_names[closest_hit_bone_idx], sv_skeleton->model);
...
void PF_processmodelevents(void) {
    ...
    // uint16_t *framegroup_n_events = get_unpacked_member(anim_skel_model->framegroup_n_events, anim_skel_model);
    ...
    //     return;
    // }
    // float **framegroup_event_time = get_unpacked_member(anim_skel_model->framegroup_event_time, anim_skel_model);
    // uint32_t **framegroup_event_code = get_unpacked_member(anim_skel_model->framegroup_event_code, anim_skel_model);
    // char ***framegroup_event_data_str = get_unpacked_member(anim_skel_model->framegroup_event_data_str, anim_skel_model);
    // // --
    // float *anim_event_times = get_unpacked_member(framegroup_event_time[anim_framegroup_idx], anim_skel_model);
    // uint32_t *anim_event_codes = get_unpacked_member(framegroup_event_code[anim_framegroup_idx], anim_skel_model);
    // char **anim_event_data_strs = get_unpacked_member(framegroup_event_data_str[anim_framegroup_idx], anim_skel_model);
    ...
    //             char *event_data_str = get_unpacked_member(anim_event_data_strs[event_idx], anim_skel_model);
