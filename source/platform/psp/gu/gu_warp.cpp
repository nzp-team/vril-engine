...
void Sky_LoadSkyBox (char *name)
{
    ...
    for (int i = 0; i < 4; i++)
    {
		...
		if(!(skyimage[i] = loadskyboxsideimage (va("gfx/env/%s%s", name, suf[i]), IMAGE_TGA | IMAGE_PNG | IMAGE_JPG, false, GU_LINEAR)) &&
           !(skyimage[i] = loadskyboxsideimage (va("gfx/env/%s_%s", name, suf[i]), IMAGE_TGA | IMAGE_PNG | IMAGE_JPG, false, GU_LINEAR)))
		{
			Con_Printf("Sky: %s[%s] not found, used std\n", name, suf[i]);
			if(!(skyimage[i] = loadskyboxsideimage (va("gfx/env/skybox%s", suf[i]), IMAGE_TGA | IMAGE_PNG | IMAGE_JPG, false, GU_LINEAR)))
		    {
