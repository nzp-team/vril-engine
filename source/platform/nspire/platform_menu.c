...
{
	struct dirent *dp;
    dp = opendir(va("%s/maps", com_gamedir)); // Open the directory - dir contains a pointer to manage the dir
	struct dirent *ep;
	...
	while((ep = readdir(dp)))
	{
		...
		if(!strcmp(COM_FileExtension(ep->d_name),"bsp")|| !strcmp(COM_FileExtension(ep->d_name),"BSP"))
	    {
			char ntype[32];

			COM_StripExtension(ep->d_name, ntype);
			custom_maps[num_user_maps].occupied = true;
			custom_maps[num_user_maps].map_name = malloc(sizeof(char)*32);
			sprintf(custom_maps[num_user_maps].map_name, "%s", ntype);

			char* 		setting_path;

			setting_path 								  	= malloc(sizeof(char)*64);
			custom_maps[num_user_maps].map_thumbnail_path 	= malloc(sizeof(char)*64);
			...
			if (setting_file != NULL) {
				...
				custom_maps[num_user_maps].map_name_pretty = malloc(sizeof(char)*32);
				custom_maps[num_user_maps].map_desc[0] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[1] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[2] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[3] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[4] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[5] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[6] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_desc[7] = malloc(sizeof(char)*40);
				custom_maps[num_user_maps].map_author = malloc(sizeof(char)*40);
				...
				while(buffer != NULL) {
					switch(state) {
						...
						case 10: sscanf(remove_windows_newlines(buffer), "%d", &value); custom_maps[num_user_maps].map_use_thumbnail = value; break;
						case 11: sscanf(remove_windows_newlines(buffer), "%d", &value); custom_maps[num_user_maps].map_allow_game_settings = value; break;
						default: break;
					}
					state++;
					buffer = strtok(NULL, "\n");
	...
	custom_map_pages = (int)ceil((double)(num_user_maps)/15);
}
