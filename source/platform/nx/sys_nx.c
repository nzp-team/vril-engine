...
{
	Con_Printf ("Nintendo Switch NZP v%4.1f (3DSX: "__TIME__" "__DATE__")\n", (double)(VERSION));
}
...
int main(int argc, char *argv[]) {
    ...
    dir = opendir(GAMENAME);
    if (!dir) Sys_Error("could not open `" GAMENAME "`");
    havebase = 1;
    closedir(dir);

    if (!havebase)
        Sys_Error("base game directory `nzp` not found in `%s`", GAMENAME);
