...
void
R_CalcBeamVerts(float * vert, vec3_t org1, vec3_t org2, float width); // Vult
...
qboolean
OnChange_gl_particle_count(cvar_t * var, char * string)
{
    float f;
...
void
R_CalcBeamVerts(float * vert, vec3_t org1, vec3_t org2, float width);
void
QMB_ColorMappedExplosion(vec3_t org, int colorStart, int colorLength);

extern cvar_t sv_gravity;

static byte *
ColorForParticle(part_type_t type)
{
    static col_t color;
    int lambda;
...
R_InitParticles(void)
{
    ...
    ADD_PARTICLE_TYPE(p_spark, pd_spark, HYE_SRC_ALPHA, HYE_ONE, ptex_none, 255, -8, 0, pm_normal, 1.3);
    ADD_PARTICLE_TYPE(p_gunblast, pd_spark, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_none, 255, 0, 0, pm_normal,
      1.3);
    ADD_PARTICLE_TYPE(p_sparkray, pd_sparkray, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_none, 255, -0, 0,
      pm_nophysics, 0);
    ADD_PARTICLE_TYPE(p_fire, pd_billboard, HYE_SRC_ALPHA, HYE_ONE, ptex_smoke, 204, 0, -2.95, pm_die, 0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_fire2, pd_billboard, HYE_SRC_ALPHA, HYE_ONE, ptex_smoke, 204, 0, -2.95, pm_die, 0);
    ADD_PARTICLE_TYPE(p_chunk, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 255, -16, 0,
      pm_bounce, 1.475);
    ADD_PARTICLE_TYPE(p_shockwave, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 255, 0, -4.85,
      pm_nophysics, 0);
    ADD_PARTICLE_TYPE(p_inferno_flame, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 153, 0, 0,
      pm_static, 0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_inferno_trail, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 204, 0, 0,
      pm_die, 0);
    ...
    ADD_PARTICLE_TYPE(p_dpsmoke, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_dpsmoke, 85, 3, 0, pm_die,
      0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_dot, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 255, 0, 0, pm_static,
      0);
    ADD_PARTICLE_TYPE(p_blood1, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_COLOR, ptex_blood1, 255, -20, 0, pm_die,
      0);
    ADD_PARTICLE_TYPE(p_blood2, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_blood3, 255, -45, 0,
      pm_normal, 0.018); // disisgonnabethegibchunks
    ADD_PARTICLE_TYPE(p_blood3, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_blood3, 255, -30, 0,
      pm_normal, 0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_flame, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 200, 10, 0, pm_die,
      0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_lavatrail, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_dpsmoke, 255, 3, 0,
      pm_normal, 0); // R00k
    ADD_PARTICLE_TYPE(p_glow, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 204, 0, 0, pm_die, 0);
    ADD_PARTICLE_TYPE(p_alphatrail, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 100, 0, 0,
      pm_static, 0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_torch_flame, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_flame, 255, 12, 0,
      pm_die, 0);
    ADD_PARTICLE_TYPE(p_streak, pd_hide, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_none, 255, -64, 0, pm_streak,
      1.5);
    ADD_PARTICLE_TYPE(p_streakwave, pd_hide, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_none, 255, 0, 0,
      pm_streakwave, 0);
    ADD_PARTICLE_TYPE(p_streaktrail, pd_beam, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_none, 255, 0, 0, pm_die, 0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_lightningbeam, pd_beam, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_lightning, 255, 0, 0,
      pm_die, 0);
    ADD_PARTICLE_TYPE(p_muzzleflash, pd_billboard, HYE_SRC_ALPHA, HYE_ONE, ptex_muzzleflash, 255, 0, 0, pm_static, 0);
    ADD_PARTICLE_TYPE(p_muzzleflash2, pd_billboard, HYE_SRC_ALPHA, HYE_ONE, ptex_muzzleflash2, 255, 0, 0, pm_static, 0);
    ADD_PARTICLE_TYPE(p_muzzleflash3, pd_billboard, HYE_SRC_ALPHA, HYE_ONE, ptex_muzzleflash3, 255, 0, 0, pm_static, 0);
    ADD_PARTICLE_TYPE(p_rain, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_generic, 255, -16, 0, pm_rain,
      0);

    loading_cur_step++;
    SCR_UpdateScreen();

    ADD_PARTICLE_TYPE(p_bloodcloud, pd_billboard, HYE_SRC_ALPHA, HYE_ONE_MINUS_SRC_ALPHA, ptex_bloodcloud, 255, -2, 0,
      pm_normal, 0);
...
__inline static void
AddParticleTrail(part_type_t type, vec3_t start, vec3_t end, float size, float time, col_t col)
{
    byte * color;
...
void
QMB_Shockwave_Splash(vec3_t org, int radius)
{
    float theta;
    vec3_t angle;
...
void
d8to24col(col_t colourv, int colour)
{
    byte * colourByte;
...
__inline static void
AddColoredParticle(part_type_t type, vec3_t org, int count, float size, float time, int colorStart, int colorLength,
  vec3_t dir)
{
    col_t color;
    int i, j, colorMod = 0;
...
void
QMB_Blood_Splat(part_type_t type, vec3_t org) // Shpuldified
{
    int j;
    col_t color;
    vec3_t neworg, angle;
...
void
QMB_FlameGt(vec3_t org, float size, float time)
{
    if (fabs(cl.ctime - cl.oldtime))
...
void
QMB_Q3TorchFlame(vec3_t org, float size)
{
...
void
QMB_Lightning_Splash(vec3_t org)
{
    int i, j;
    vec3_t neworg, angle;
    col_t color;
...
void
QMB_LightningBeam(vec3_t start, vec3_t end)
{
    float frametime = fabs(cl.time - cl.oldtime);
...
void
R_SetParticleMode(part_mode_t val)
{
    particle_mode = val;
...
CheckDecals(void)
{
    if (!r_decal_bullets.value && !r_decal_explosions.value && !r_decal_sparks.value && !r_decal_blood.value)
        decals_enabled = 0;
    else if (r_decal_bullets.value && r_decal_explosions.value && r_decal_sparks.value && r_decal_blood.value)
        decals_enabled = 1;
    else
        decals_enabled = 2;
}
